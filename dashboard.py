...
if not df_filtrado.empty and num_periodos > 0 and num_periodos % 2 == 0:
    ...
    if not df_queda.empty:
        ...
        # 10.5. GARANTIA DE FORMATAÇÃO (PADRÃO BRASILEIRO)
        colunas_a_formatar = [col for col in df_final.columns if 'Período' in col or 'Queda' in col]
        
        for col in colunas_a_formatar:
            # Lógica de formatação brasileira (9.999,99)
            df_final[col] = df_final[col].map(lambda x: f'{x:{FORMATO_NUMERICO}}'.replace(',', 'X').replace('.', ',').replace('X', '.'))
        ...
        st.success(f"Nenhum cliente no conjunto filtrado teve queda no {LABEL_METRICA} ao comparar o Período 1 com o Período 2.")
